...
function normalizeSection(sectionParam?: string): DashboardSection {
  ...
  if (
    !sectionParam ||
    !accepted.has(sectionParam as DashboardSection)
  ) {
    return fallback;
  }
...
function formatTelefoneDisplay(
  digits: string
): string {
...
function priorityVariant(
  priority: HelpdeskTicket["prioridade"]
) {
  if (
    priority === "muito alta" ||
    priority === "alta"
        ...
        <Card>
          <CardContent className="space-y-5 py-5">
            <div>
              ...
              <p className="mt-1 capitalize text-stone-200">
                {chamado.status}
              </p>
            </div>
...
export function DashboardPage({
  ...
  const chamadoSelecionadoId =
    Number(searchParams.get("chamado"));
  ...
  const allowedSections =
    sectionVisibilityByRole[authUser.nivel] ??
    sectionVisibilityByRole.usuario;
  ...
  const [ticketForm, setTicketForm] =
    useState<CreateChamadoInput>(
      createInitialTicketForm(authUser.id)
    );
  ...
  const handleUpdateStatus = async (
    ...
    try {
      ...
      alert(
        erro.message ||
          "Erro ao atualizar o status do chamado."
      );
    } finally {
  ...
  navigate(
    `/dashboard/${section}?chamado=${ticket.id}`
  );
}
  ...
  async function handleUserSubmit(
    ...
    try {
      const cpfDigits =
        userForm.cpf.replace(
          /\D/g,
          ""
        );

      const rawPhoneDigits =
        userForm.telefone.replace(
          /\D/g,
          ""
        );

      const phoneDigits =
        rawPhoneDigits.startsWith(
          "55"
        ) &&
        rawPhoneDigits.length === 13
      ...
      if (
        ...
        throw new Error(
          "Telefone inválido. Use DDD + 9 dígitos (ex: 11999998888 ou +55 11 99999-8888)."
        );
      }
      ...
      if (
        ...
        throw new Error(
          "Senha inválida. Use 8+ caracteres com maiúscula, minúscula, número e símbolo."
        );
      }
      ...
      if (
        ...
        setUserSubmitError(
          "Não foi possível criar usuário. Verifique se email, CPF ou telefone já estão cadastrados, ou se os dados estão no formato esperado."
        );
      } else {
        setUserSubmitError(
          message
  ...
  async function handleProfileSubmit(
    ...
    try {
      const result =
        await updateMeuTelefone(
          profilePhone
        );

      setProfilePhone(
        result.telefone.replace(
          /\D/g,
          ""
        )
      );
                    ...
                    <Card>
                      <CardContent className="space-y-4 py-4">
                        <div>
                          ...
                          <p className="text-sm text-stone-400">
                            Envie um novo ticket para o backend.
                          </p>
                        </div>
                    ...
                    <Card>
                      <CardContent className="space-y-4 py-4">
                        <div>
                          ...
                          <p className="text-sm text-stone-400">
                            Visualize seus dados e mantenha seu telefone atualizado.
                          </p>
                        </div>
